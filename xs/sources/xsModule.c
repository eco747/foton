...
void fx_require(txMachine* the)
{
	...
	mxTry(the) {
		...
		exports = mxModuleExports(module)->value.reference->next;
		while (exports) {
			mxCheck(the, exports->kind == XS_EXPORT_KIND);
			if (exports->ID == defaultID) {
				mxResult->kind = exports->value.exports.closure->kind;
				mxResult->value = exports->value.exports.closure->value;
				break;
			}
					...
					if (star->ID != mxID(_default)) {
						...
						if (ambiguous) {
							...
							if (ambiguous->kind == XS_EXPORT_KIND)
								ambiguousModule = ambiguous->value.exports.module;
							else
								ambiguousModule = mxTransferClosure(ambiguous)->value.exports.module;
							if (star->kind == XS_EXPORT_KIND)
								starModule = star->value.exports.module;
							else
								starModule = mxTransferClosure(star)->value.exports.module;
							if (ambiguousModule != starModule) {
								ambiguous->kind = XS_EXPORT_KIND;
								ambiguous->value.exports.closure = C_NULL;
								ambiguous->value.exports.module = C_NULL;
							}
						}
						else {
							export = export->next = fxNewSlot(the);
		...
		while (transfer) {
			...
			if (local->kind != XS_NULL_KIND) {
				...
				if ((from->kind == XS_NULL_KIND) || (import->kind == XS_NULL_KIND)) {
					closure = mxTransferClosure(transfer);
					closure->value.exports.closure = fxNewSlot(the);
					closure->value.exports.closure->kind = XS_UNINITIALIZED_KIND;
					closure->value.exports.module = module->value.reference;
		...
		if (function->kind == XS_REFERENCE_KIND) {
			...
			while (transfer) {
				...
				if (local->kind != XS_NULL_KIND) {
					...
					closure->value.closure = export->value.exports.closure;
				}
				transfer = transfer->next;
			}
...
void fxResolveNamespace(txMachine* the, txID fromID, txSlot* transfer)
{
	...
	mxCheck(the, export->kind == XS_EXPORT_KIND);
	export->value.exports.closure = fxGetModule(the, fromID);
}
...
void fxResolveTransfer(txMachine* the, txID fromID, txID importID, txSlot* transfer)
{
	...
	if (export) {
		if (export->kind == XS_EXPORT_KIND) {
			if (export->value.exports.closure == C_NULL) {
				txString path = C_NULL;
				txInteger line = 0;
			#ifdef mxDebug
...
txSlot* fxModuleGetProperty(txMachine* the, txSlot* instance, txID id, txIndex index, txFlag flag)
{
	...
	if (mxIsReference(exports)) {
		...
		while (property) {
			if (property->ID == id) {
				property = property->value.exports.closure;
...
void fxModuleOwnKeys(txMachine* the, txSlot* instance, txFlag flag, txSlot* keys)
{
	...
	if (mxIsReference(exports)) {
		if (flag & XS_EACH_NAME_FLAG) {
			...
			while (property) {
				if (property->value.exports.closure) {
