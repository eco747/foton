...
static bool attribute_matches( const GumboVector* attributes, const char* name, const char* value) {
	...
	//<eco:  return attr ? strcasecmp(value, attr->value) == 0 : false;
...
//<eco:        (!exact_match && !strcasecmp(needle, haystack[i].data))) {
...
static bool node_qualified_tag_is(const GumboNode* node, GumboNamespaceEnum ns, GumboTag tag) {
  assert(node);
  return (node->type == GUMBO_NODE_ELEMENT || node->type == GUMBO_NODE_TEMPLATE) && node->v.element.tag == tag && node->v.element.tag_namespace == ns;
}
...
static bool is_mathml_integration_point(const GumboNode* node) {
	return node_tag_in_set( node, (gumbo_tagset){TAG_MATHML(MI), TAG_MATHML(MO), TAG_MATHML(MN), TAG_MATHML(MS), TAG_MATHML(MTEXT)});
}
...
static bool is_html_integration_point(const GumboNode* node) {
  return node_tag_in_set(node, (gumbo_tagset){TAG_SVG(FOREIGNOBJECT), TAG_SVG(DESC), TAG_SVG(TITLE)}) ||
		 (node_qualified_tag_is( node, GUMBO_NAMESPACE_MATHML, GUMBO_TAG_ANNOTATION_XML) &&
			 (attribute_matches(&node->v.element.attributes, "encoding", "text/html") || attribute_matches(&node->v.element.attributes, "encoding", "application/xhtml+xml")));
}
...
InsertionLocation get_appropriate_insertion_location( GumboParser* parser, GumboNode* override_target) {
	...
	if (!parser->_parser_state->_foster_parent_insertions || !node_tag_in_set(retval.target, (gumbo_tagset){TAG(TABLE), TAG(TBODY),TAG(TFOOT), TAG(THEAD), TAG(TR)})) {
		return retval;
	}
...
static GumboNode* pop_current_node(GumboParser* parser) {
	...
	if((state->_current_token->type != GUMBO_TOKEN_END_TAG || !node_html_tag_is(current_node, state->_current_token->v.end_tag)) &&
...
void clear_stack_to_table_body_context(GumboParser* parser) {
	while (!node_tag_in_set(get_current_node(parser), (gumbo_tagset){TAG(HTML), TAG(TBODY), TAG(TFOOT), TAG(THEAD), TAG(TEMPLATE)})) {
		pop_current_node(parser);
	}
}
