...
litehtml::element::ptr litehtml::document::create_element(const tchar_t* _tag_name, const attr_map& attributes)
{
	element::ptr newTag;
	...
	if(m_container)	{
		newTag = m_container->create_element(_tag_name, attributes, this_doc);
	}
	...
	if(!newTag)
	{
		...
		else if( tag_name==atom_p )		{ newTag = std::make_shared<litehtml::el_para>(this_doc); }
		else if( tag_name==atom_img )	{ newTag = std::make_shared<litehtml::el_image>(this_doc); }
		else if(tag_name==atom_table)	{ newTag = std::make_shared<litehtml::el_table>(this_doc); }
		else if(tag_name==atom_td || tag_name==atom_th ) { newTag = std::make_shared<litehtml::el_td>(this_doc); }
		else if( tag_name==atom_link )	{ newTag = std::make_shared<litehtml::el_link>(this_doc); }
		else if(tag_name==atom_title)	{ newTag = std::make_shared<litehtml::el_title>(this_doc); }
		else if( tag_name==atom_a )		{ newTag = std::make_shared<litehtml::el_anchor>(this_doc); }
		else if(tag_name==atom_tr)		{ newTag = std::make_shared<litehtml::el_tr>(this_doc); }
		else if(tag_name==atom_style)	{ newTag = std::make_shared<litehtml::el_style>(this_doc); }
		else if(tag_name==atom_base )	{ newTag = std::make_shared<litehtml::el_base>(this_doc); }
		else if(tag_name==atom_body)	{ newTag = std::make_shared<litehtml::el_body>(this_doc); }
		else if(tag_name==atom_div)		{ newTag = std::make_shared<litehtml::el_div>(this_doc); }
		else if(tag_name==atom_script)	{ newTag = std::make_shared<litehtml::el_script>(this_doc); }
		else if(tag_name==atom_font )	{ newTag = std::make_shared<litehtml::el_font>(this_doc); }
		else							{ newTag = std::make_shared<litehtml::html_tag>(this_doc); }
	}

	if(newTag)
	{
		newTag->set_tagName(tag_name);
		for (attr_map::const_iterator iter = attributes.begin(); iter != attributes.end(); iter++)
		{
	...
	case GUMBO_NODE_ELEMENT:
		{
			...
			for (unsigned int i = 0; i < node->v.element.attributes.length; i++)
			{
				...
				attrs[atom_create(litehtml_from_utf8(attr->name))] = litehtml_from_utf8(attr->value);
			}
			...
			if (ret)
			{
				...
				elements.push_back( element::ptr(ret) );
			}
		}
		break;
...
void litehtml::document::fix_flex_layouts()
{
	size_t i = 0;
