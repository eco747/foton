...
const litehtml::tchar_t* litehtml::el_text::get_style_property( atom name, bool inherited, const tchar_t* def /*= 0*/ )
{
	if(inherited)
	{
...
void litehtml::el_text::draw( uint_ptr hdc, int x, int y, const position* clip )
{
	...
	if(pos.does_intersect(clip))
	{
		...
		if (el_parent)
		{
			...
			litehtml::web_color color = el_parent->get_color(atom_color, true, doc->get_def_color());
