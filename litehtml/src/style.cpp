...
litehtml::attr_map litehtml::style::m_valid_values =
{
...
void litehtml::style::add_property( atom name, const tchar_t* val, const tchar_t* baseurl, bool important )
{
	if(!name || !val)
	{
		return;
...
void litehtml::style::add_parsed_property( atom name, const tstring& val, bool important )
{
	bool is_valid = true;
...
void litehtml::style::remove_property( atom name, bool important )
{
	props_map::iterator prop = m_properties.find(name);
	if(prop != m_properties.end())
	{
